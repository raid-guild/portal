...
Rule: direct `POST /api/events` creates only the Portal record. `syncDiscord` is not a persisted field and is ignored by the raw Payload collection endpoint. Agents creating future sessions must default to `POST /api/events/create` with `syncDiscord: true` and confirm the response has `discordSyncStatus: synced`, unless the user explicitly asked for a Portal-only session, the session is a past/imported record, or an existing Discord event URL is supplied.
