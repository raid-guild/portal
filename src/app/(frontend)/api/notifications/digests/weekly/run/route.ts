...
import { createWeeklyDigestNotifications } from '@/notifications/createWeeklyDigestNotifications'

export async function POST(request: Request) {
  const configuredSecret = process.env.AGENT_REGISTRATION_SECRET

  if (!configuredSecret) {
    return Response.json({ message: 'Weekly digest endpoint is not enabled.' }, { status: 404 })
  }
